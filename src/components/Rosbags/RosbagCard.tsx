...
import { Badge, Card, List, Heading, HStack, Text, Tag, Show, Box, Checkbox, Button, Flex } from "@chakra-ui/react"
  ...
  return (
    <Card.Body flexDir="column" spaceY="2">
      ...
      <Card.Description flexDir="column">
        <Show when={expanded}>
          <Heading size="md" mt="5" mb="2">Topics </Heading>
          <List.Root pl="5">
      ...
      <Flex justifyContent="flex-end" w="100%">
        <Button variant="outline" size="xs" h="7" onClick={() => { setExpand(!expanded) }}>{expanded ? "less" : "more"}</Button>
      </Flex>
    </Card.Body>
  );
}
  ...
  <Card.Footer flexDir="column" spaceY="2">
    ...
    <HStack wrap="wrap" w="100%">
      {rosbag.tags?.map((tag, key) => (
        <Badge variant="outline" textStyle="sm" key={key}>{tag}</Badge>
      ))}
    </HStack>
