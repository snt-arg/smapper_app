import { Button, Badge, CloseButton, Container, Dialog, Field, Fieldset, Input, Portal, Stack, Table, Textarea, useDisclosure } from "@chakra-ui/react";
...
export default function RosbagsTable() {
  ...
  const getRosbags = async () => {
    try {
      const data = await fetchRosbags(); setRosbags(data);
    } catch (error) {
